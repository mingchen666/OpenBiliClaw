...
- `[scheduler].pool_target_count` 默认从 600 降到 300；初始化关注 / 订阅类信号默认收敛到 100，B 站关注、XHS / Douyin / YouTube `bootstrap_profile` 的 `max_items_per_scope` 和插件 fallback 保持同一默认，减少首次画像和后台补池的无谓事件量。
