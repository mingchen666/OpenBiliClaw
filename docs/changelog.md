...
- 插件头部窄宽度对齐修复 + GitHub Star 按钮：side panel 默认窄宽（<460px）下头部不再把操作图标挤到品牌下方、右浮成空一截的第二行；改为**始终单行**布局（品牌左、图标右、整体垂直居中），窄屏隐藏装饰性 eyebrow、状态徽标仅在空间不足时紧凑换到标题下、图标压到 28px、品牌标题用 `clamp()` 流体字号保证「OpenBiliClaw」整词不被裁，宽屏（≥460px）维持原样。并在头部图标栏首位新增**常驻的金色 ★ GitHub Star 按钮**（金底金星、在一排灰色图标里凸显，点击打开 repo），替代早先一版的可关闭横幅。用 chrome-devtools 在 360 / 400 / 560px 实测三档对齐与五图标容纳（含修复 `.webui-button{width:32px}` 因源码靠后盖过 `@media` 压缩规则的坑，用 `.hero-actions button` 提高优先级）；`tests/popup-layout.test.ts` 改判新单行布局 + 常驻 Star 按钮断言。
