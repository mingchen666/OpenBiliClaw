...
| 头部响应式 + Star 按钮 | ✅ | side panel 默认窄宽（<460px）下头部保持**单行**（品牌左、操作图标右、垂直居中）：隐藏装饰 eyebrow、状态徽标空间不足时紧凑换到标题下、图标压 28px、标题用 `clamp()` 流体字号防裁词，不再把图标右浮成空一截的第二行；≥460px 维持原样。头部图标栏首位有**常驻金色 ★ GitHub Star 按钮**（金底金星，点击开 repo，在灰色图标里凸显）。响应式断点用 chrome-devtools 实测 360/400/560px，断言在 `tests/popup-layout.test.ts` |
