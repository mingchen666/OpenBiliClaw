...
| 5.6 发现引擎编排 | 🔄 | 已能运行 Search/Trending/RelatedChain，待补并行和缓存 |
...
6. **评分入口集中在引擎层**：`ContentDiscoveryEngine.evaluate_content()` 统一负责把 `score/reason` 写回 `DiscoveredContent`
7. **引擎层仍不负责依赖创建**：`ContentDiscoveryEngine` 接收外部注入的 `llm_service`，策略继续显式注入 client/service
