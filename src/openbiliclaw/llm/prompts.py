...
6. 所有 query 的核心主题词（第一个实词）必须两两不同，
   禁止同一概念换皮出现多次。
7. 反信息茧房硬性约束：列表中最后 2 个 query 必须是"完全跨域"——
   其核心主题词与用户所有兴趣域/兴趣词条不得有语义关联
   （包括同义词、上下位词、同一学科分支）。
   如果一个 query 的主题可以被归入用户任一兴趣的上位分类，就不算完全跨域。
   合格示例（用户关注 AI/编程/科技）：
     "手工皮具 制作过程 vlog" / "地质学 野外考察 科普" / "昆虫 微距摄影 讲解"
   不合格示例："认知科学 决策"（与 AI 认知距离过近）/ "数据可视化 设计"（编程相关）
</rules>
